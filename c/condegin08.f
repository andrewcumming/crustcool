...
** Modification WITH FINITE SIZES OF NUCLEI; xnuc=r_{nuc}/a_i
     ...
     *  (1.d0+xnuc**2*dsqrt(2.d0*Zion))
      ...
      if (TRU.lt.20.) then ! correction for dying-out umklapp processes
         CLhigh=CLeff
         EU=dexp(-1.d0/TRU)
* CLlowK, CLlowS corrected on 23.05.07 following A.Chugunov's remark
         CLlowK=15.918*sqrt(XSR/CMI/Zion)*TRP**3 ! low-T lim.for kappa
         CLlowS=CLlowK/VCL/BOHR*PI**2/.75*TRP**2 ! low-T lim.for sigma
...
*	print*,CL1,V**2,CL2,CL1-V**2*CL2
        if (I.eq.0) then ! 1st term calculated
           COULAN2=CL1-V**2*CL2
*	print*,'COULAN2=',COULAN2
