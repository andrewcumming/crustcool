...
public:
  ...
  void finit(FILE *fp, int ycol, int ncol, int n);
...
double Spline::get(double x)
{
  ...
  if (x <= this->xtab[1]) {
    ...
    if (this->out_of_bounds_flag == 0) return 0.0;
      ...
      if (this->log_flag==0) return this->ytab[1];
  ...
  if (x >= this->xtab[this->num]) {
    ...
    if (this->out_of_bounds_flag == 0) return 0.0;
      ...
      if (this->log_flag==0) return this->ytab[this->num];
...
void Spline::finit(FILE *fp, int ycol, int ncol, int n)
{
  double d1,d2,dummy;
  int i,col;
  ...
  for(i=1; i<=n; i++) {
    ...
    fscanf(fp, "%lg",&(this->xtab[i]));
 col=1;
    while (++col != ycol)     fscanf(fp, "%lg",&dummy);
    //fread(&dummy,1,DSIZE,fp);
    //fread(&(this->ytab[i]),1,DSIZE,fp);
     fscanf(fp, "%lg",&(this->ytab[i]));
   while (col++ != ncol)    fscanf(fp, "%lg",&dummy);
   // fread(&dummy,1,DSIZE,fp);
    //    printf("x=%lg, y=%g\n", this->xtab[i], this->ytab[i]);
  }
