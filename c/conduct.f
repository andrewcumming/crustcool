...
*                                                       Version 17.02.08
      subroutine CONDUCT(TEMP,DENSI,B0,Zion,CMI,Zimp,
     & RSIGMA,RLET,RLTT,RQ,RKAPPA,
     & RTSIGMA,RTLET,RTLTT,RTQ,RTKAPPA,
     & RHSIGMA,RHLET,RHLTT,RHQ,RHKAPPA)
* Input: TEMP - temperature, DENSI - number density of ions, B0- magn.f.
...
*   -------------------   RESTRICTIONS:   --------------------------   *
*      if (TEMP.le.0..or.DENSI.le.0..or.B0.lt.0..or.Zion.le.0.
*     & .or.CMI.le.0.) then
*         print*,'TEMP,DENSI,B0,Zion,CMI:',TEMP,DENSI,B0,Zion,CMI
...
*   XSR equals the non-magnetic Fermi momentum
      if (B0.gt.0.) then
         CNL=((sqrt(1.+XSR**2)+3.*TEMP)**2-1.)/2./B0
      else
         CNL=10000.
      endif
...
*   -------------------  Chemical potential   ----------------------   *
      CMU=CHEMPOT(B0,DENS,DDENS,TEMP)
      Q2e=4.*PI/BOHR*DDENS ! squared electron screening momentum
*   -------------------  Thermal averaging    ----------------------   *
      call ThAv99(MAGNET,CMU,B0,TEMP,XSR,GAMMA,Zion,CMI,Zimp,Q2e,
...
*   Longitudinal transport coefficients:
      ...
      RLTT=F2B*TEMP ! thermal transport coeff.
      RQ=RLET/RSIGMA ! thermopower
      RKAPPA=RLTT-TEMP*RLET**2/RSIGMA ! thermal conductivity
...
*     ... thermal conductivity
      RTKAPPA=RTLTT-TEMP*(RTLET*RTQ-RHLET*RHQ) ! diag.
      RHKAPPA=RHLTT-TEMP*(RTLET*RHQ+RHLET*RTQ) ! non-diag.
*   ------  Include nonmagnetic corrections for thermal conductivity:
      if (B0.eq.0.) then
* (1) correction for e-e collisions:
         CTH=TEMP/9.*XSR**3/sqrt(1.+XSR**2)
...
* (2) correction for ionic quantum effects (W-F law violation at low T)
         ...
         G0=TRP/sqrt(TRP**2+T0**2)*(1.+(Zion/125.)**2)
         G2=TRP/sqrt(.0081+TRP**2)**3
         THtoEL=1.+G2/G0*(1.+BORNCOR*VCL**3)*.0105*(1.-1./Zion)
* result:
         RKAPPA=RKAPPA*EECOR/THtoEL
         RTKAPPA=RTKAPPA*EECOR/THtoEL
         RHKAPPA=RHKAPPA*EECOR/THtoEL
      endif
      return
      end

      subroutine ThAv99(MAGNET,CMU,B,TEMP,XSR,GAMMA,Zion,CMI,Zimp,Q2e,
     *   F0,F1,F2,F0C,F1C,F2C,F0D,F1D,F2D)
*   Thermal Averaging                                   Version 12.05.11
*    MAGNET=1 - magnetic case, 0 - non-magnetic case
*    CMU - chemical potential, B -magn.field, TEMP -temperature (mc^2)
...
*      if (MAGNET.ne.0.and.MAGNET.ne.1) stop'ThAv: incorrect MAGNET'
      ...
      EMIN=dmax1(1.d0,CMU-TAIL2*TEMP)
      EMAX=CMU+TAIL2*TEMP
      E1=dmax1(1.d0,CMU-TAIL1*TEMP)
      E2=dmax1(1.d0,CMU+TAIL1*TEMP)
      if (MAGNET.eq.1) then
         NLMIN=(EMIN**2-1.)/2./B
         NLMAX=(EMAX**2-1.)/2./B
         NL1=(E1**2-1.)/2./B
...
*  ----------------   Calculation of the integrand   ----------------  *
  100 continue
*  CMU, TEMP, XSR, GAMMA, B, Zion, CMI, Q2e, E should be defined
      T=(E-CMU)/TEMP
...
*                                                       Version 03.05.07
      ...
      if (PF0.gt..001) then
         THETA=TEMP/(sqrt(1.+PF0**2)-1.) ! degeneracy
      else
         THETA=2.*TEMP/PF0**2
      endif
      if (B0.gt.0.) then
         CNL=((sqrt(1.+PF0**2)+6.*TEMP)**2-1.)/2./B0 ! 25.11.96
      else
         CNL=10000.
      endif
*   CNL estimates maximum contributing Landau number
...
*   ------------------  Fit to chemical potential  -----------------   *
         CHEMPOT=CHEMFIT(DENS,DDENS,TEMP)
         return
      endif
*   ----------  Calculation of chemical potential CMU   ------------   *'
      ...
      DENS1=0. ! 22.01.97
      do 5 I=1,NI
      if (ZET.gt.3.*TEMP) then
*  Degenerate plasma
         SHIFTMU=0.
      else
*  Non-degenerate plasma
         SHIFTMU=1.5*TEMP*dlog(3.*TEMP/ZET)
      ...
      DENS1old=DENS1
      call DENSFIT(B,TEMP,CMU,DENS1,DDENS)
...
*   55 print*,'Warning CHEMPOT: INCORRECT density:',DENS1,'  i.o.',DENS
    6 continue
      ...
      function CHEMFIT(DENS,DDENS,TEMP)
      implicit double precision (A-H), double precision (O-Z)
      save
...
*   are used here only for the derivative dn/d\mu, not for \mu.
      XSR=(3.*PI**2*DENS)**.3333333 ! Classical Fermi momentum
      if (XSR.gt..001) then
         THETA=TEMP/(dsqrt(1.+XSR**2)-1.) ! degeneracy parameter
      else
         THETA=2.*TEMP/XSR**2
      endif
*   -------------- NEW FIT to the chemical potential ---------------   *
     ...
     /(6.192*THETA**.0944/T+5.535*T*THETA**.698)
      ...
      UP=1.+P01*TEMP*P02+P03*sqrt(TEMP)
      DN=(1.+.5*TEMP/THETA)*(1.+P01*TEMP)
      CT=1.+UP/DN*TEMP
         F=.66666667/THETA/dsqrt(THETA)
         X1=FERINV(F,1) ! non-relativistic result
     -     -    1.5*dlog(CT) ! Relativistic fit
         CMU1=TEMP*X1 ! Fit to chemical potential w/o mc^2
         CHEMFIT=CMU1+1.
*  Fit to the DERIVATIVE of chemical potential:
         THETAlog=dlog(THETA)
         THETACOR=(THETA32+0.29)**.666667
         TB=dexp(-B1ICH*THETAlog)   ! = THETA^{-b}
         TB1=TB/THETA           ! = THETA^{-(b+1)}
         DENOM=(1.+AICH*TB)
         UP=(AICH*TB1+BICH*dsqrt(TB1))
        DMT=-1.5-
     -   (B1ICH+1.)*(AICH*TB1+BICH/2.*dsqrt(TB1))/DENOM+
     +   UP*B1ICH*AICH*TB/DENOM**2 ! d (mu/T) / d (ln theta)
         DTP=-THETA/TEMP*XSR**2/dsqrt(1.+XSR**2) ! d(ln theta)/d(ln p_F)
* INCLUDE RELATIVISTIC CORRECTION :
         DMT=DMT-
     -     1.5*THETA32*TEMP**2/
     /     ((1.+2.*TEMP)*THETACOR+TEMP**2)/(THETA32+.27)
         DDENS=3.*DENS/(DMT*DTP)/TEMP ! d n_e / d mu
         return
      end

*  ========================   Fitting density   =====================  *
      subroutine DENSFIT(B,TEMP,CMU,DENS,DDENS)
*                                                       Version 10.10.96
* Input: Zeff, B (magn. field in 4.4*10^13 G),
*        TEMP and CMU (temperature and chemical potential in mc^2)
...
*        Max Landau number
      ...
      if (CNL0.gt.1000.) then ! non-magn. case
         TINV=1./TEMP  ! inverse relativistic temperature
         CHI=(CMU-1.)/TEMP
         W1=BLINW(1,TINV,CHI)
         W0=BLINW(0,TINV,CHI)
         G1=(W1+TINV*W0)*dsqrt(2.*TINV)
         DENS=TEMP**3*G1/PI**2
         DUMMY=CHEMFIT(DENS,DDENS,TEMP) ! determine DDENS
         goto 10
      endif
      NL0=CNL0
      EN2=1.
...
* DIVIDED by sqrt(2*TINV), - Fermi integrals if TEMP --> 0.
*                                                       Version 29.04.98
      implicit double precision (A-H), double precision (O-Z)
      save
...
*   ----------------------------------------------------------------   *
        ...
        do I=1,5
           CHIX=CHI*AX(I)
           ECHI=dexp(CHI*(AX(I)-1.))
           DS1=AH(I)*CHIX**K*dsqrt(CHI*(CHIX+ALT))/(1.+ECHI)
           CHIX=AXI(I)+CHI
           DS2=AV(I)*CHIX**K*dsqrt(CHIX*(CHIX+ALT))
           W=W+CHI*DS1+DS2
        enddo
        W=W+(WAUX-W)*dexp(-(CHI-.4)*3.) ! for smoothness (29.04.98)
*   ----------------------------------------------------------------   *
        if (CHI.lt.9.) goto 60
        WAUX=W
        W=0.
        ...
        if (P.gt.1.d-5) then
          F0=(CHI+TINV)*SQR-TINV**2*dlog(1.+(CHI+SQR)/TINV)
          F0=F0/2.
        else
          F0=SQA*dsqrt(CHI)**3/1.5
        endif
        if (K.eq.0) then
           W=F0
           goto 50
        ...
        if (P.gt..05) then
          F2=(CHI*SQR**3-5.*TINV*F1)/4.
        else
          F2=SQA*dsqrt(CHI)**7/3.5
        endif
        if (K.eq.2) then
           W=F2
           goto 50
   ...
   50    continue
         W=W+PI**2/6.*CHI**K*((K+1)*CHI+TINV*(2*K+1))/SQR
        W=W+(WAUX-W)*dexp(-.5*dsqrt(CHI-9.)) ! for smoothness (29.04.98)
   60 BLINW=W/SQA
      return
      end
...
*   ----------------------   Preliminaries   -----------------------   *
      ...
      TRP=Zion/GAMMA*dsqrt(CMI*AUM*SPHERION/3.d0/BOHR) ! =T/T_p
      TEMP=(Zion/BOHR)**2/SPHERION/GAMMA
      BORNCOR=VCL*Zion*PI/BOHR ! first non-Born correction
*   ---------------------   Non-magnetic fit   ---------------------   *
...
*   Cutoff according to Hernquist'84:
      if (GAMMA.lt.175.) then
         GAMMAN1=dsqrt(TEMP/CMI/AUM)*2.*PCL ! ei-inelasticity (liquid)
         TAUcl=PCL**2*VCL*BOHR**2/(4.*PI*Zion**2*DENSI)
      else
         GAMMAN1=TEMP/TRP ! =T_p ! ei(e-ph)-inelasticity (solid)
         TAUcl=VCL*BOHR/(TEMP*(2.-VCL**2)*Uminus2)
      endif
...
* Correction for partial degeneracy:
      ...
      subroutine CONDI(TEMP,DENSI,Zion,CMI,RKAPi)
* ion thermal conductivities in the outer crust, Chugunov & Haensel'07
...
*   ----------------------   Preliminaries   -----------------------   *
      ...
      GAMI=Zion**2/(BOHR*SPHERION*TEMP) ! corrected 06.10.08
      TRP=Zion/GAMI*dsqrt(CMI*AUM*SPHERION/3.d0/BOHR) ! =T/T_pi
      OMPI=TEMP/TRP ! ion plasma frequency
*      if (GAMI.lt.TINY**.2) pause'CONDI: GAMI is too low: kappa>HUGE'
