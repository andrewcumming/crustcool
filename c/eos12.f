...
*   rule for the Madelung energy in the random bcc multi-ion lattice.
************************************************************************
...
!f2py   intent(in) :: NMIX,AY,AZion,ACMI,RHO,TEMP
!f2py   intent(out) ::PRADnkT,PRADnkT,DENS,Zmean,CMImean,Z2mean, GAMImean,CHI,TPT,LIQSOL,PnkT,UNkT,SNk,CV,CHIR,CHIT
*                                                       Version 22.12.12
...
*         CHIT = (d ln P / d ln T)_V ("\chi_T")
      ...
      parameter (PI=3.141592653d0,C53=5.d0/3.d0,
     *   AUM=1822.888d0, 
     *   GAMIMELT=175., 
     *   RSIMELT=140.,
     *   RAD=2.554d-7) 
...
* Add Coulomb+xc nonideal contributions, and ideal free energy:
      do IX=1,NMIX
        if (AY(IX).lt.TINY) goto 10 
         Zion=AZion(IX)
         CMI=ACMI(IX)
         GAMI=Zion**C53/RS/TEMP 
         DNI=DENSI*AY(IX) 
         PRI=DNI*TEMP 
        ...
        do I=1,NMIX
          do J=I+1,NMIX
            ...
            if (X2.lt.TINY) goto 11
             ...
             GAMI=Zion**C53/RS/TEMP ! Gamma_i for given ion species
...
* the term x_i ln x_i = -S_{mix}/(N_i k).
     ...
     *     Fharm,Uharm,Pharm,CVharm,Sharm,PDTharm,PDRharm) 
...
*   Parameters adjusted to Caillol (1999).
     ...
     *  C2=-8.4d-5,G2=.0037,SQ32=.8660254038d0) 
      A3=-SQ32-A1/dsqrt(A2)
...
* ======================  AUXILIARY SUBROUTINES   ==================== *
      subroutine FERINV7(F,N,X,XDF,XDFF) 
*                                                       Version 24.05.07
